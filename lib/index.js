...
export default (options = DEFAULT_OPTIONS) => {
  ...
  return (tree) => {
    ...
    if (typeof options.postcss === 'object') {
      options.postcss = { ...POSTCSS_DEFAULT_OPTIONS, ...options.postcss };

      tree.match({ tag: 'style', attrs: { type: options.postcss.match } }, (node) => {
        return options.postcss.process(postcss([...options.postcss.plugins]).process(node.content).toString(), node, options);
      });

      return tree.filter((node, i) => (tree[0].tag === false && i !== 1))
    }
